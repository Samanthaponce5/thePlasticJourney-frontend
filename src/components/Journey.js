...
export default class Journey extends React.Component {
  ...
  listenScrollEvent = () => {
    ...
    if (currentScroll > 0 && lastScroll <= currentScroll) {
      document.body.querySelector('.not-mobile').querySelector('.seaturtle').style.transform =
        'rotateY(180deg)'
    } else {
        document.body.querySelector('.not-mobile').querySelector('.seaturtle').style.transform =
        'rotateY(360deg)'
    ...
    if (currentScroll > 10) {
      document.body.querySelector('.not-mobile').querySelector('.blob').style.opacity = '0'
      document.body.querySelector('.not-mobile').querySelector('.bubbletext').style.opacity = '0'
    } else {
      document.body.querySelector('.not-mobile').querySelector('.blob').style.opacity = '1'
      document.body.querySelector('.not-mobile').querySelector('.bubbletext').style.opacity = '1'
    }
    if (currentScroll === 0) {
      document.body.querySelector('.not-mobile').querySelector('.seaturtle').style.transform =
        'rotateY(180deg)'
    ...
    if (currentScroll == maxScrollTop) {
      ...
      document.body.querySelector('.collected2').style.width = '0'
      ...
      document.body.querySelector('.collected2').style.width = 'initial'
  ...
  handleClick = (e) => {
    ...
    if (
      e.target.className === 'plastic bottle' ||
      e.target.className === 'img b-bottle '
    ) {
      ...
      document.body
        .querySelector('.plastic.bottle')
        .classList.add('afterBottle')
      document.body
        .querySelector('.img.b-bottle')
        .classList.add('afterBottleImg')
      this.setState({ collected: collected + 1 })
    }

    if (
      e.target.className === 'plastic can' ||
      e.target.className === 'img b-can'
    ) {
    ...
    if (
      e.target.className === 'plastic cups' ||
      e.target.className === 'img b-cup'
    ) {
    ...
    if (
      e.target.className === 'plastic straw' ||
      e.target.className === 'img b-straw'
    ) {
    ...
    if (
      e.target.className === 'plastic bag' ||
      e.target.className === 'img b-bag '
    ) {
  ...
  handleMousedown = (e) => {
    if (
      e.target.className === 'plastic bottle afterBottle' ||
      e.target.className === 'img b-bottle afterBottleImg'
    ) {
      this.setState({ modalIsOpen: true })
      this.state.comparePlastics.filter((plastic) =>
        plastic.id === 3 ? this.setState({ mapPlastic: plastic }) : null,
      )
    }

    if (
      e.target.className === 'plastic bag afterBag' ||
      e.target.className === 'img b-bag afterBagImg'
    ) {
      this.setState({ modalIsOpen: true })
      this.state.comparePlastics.filter((plastic) =>
        plastic.id === 2 ? this.setState({ mapPlastic: plastic }) : null,
      )
    }

    if (
      e.target.className === 'plastic straw afterStraw' ||
      e.target.className === 'img b-straw afterStrawImg'
    ) {
      this.setState({ modalIsOpen: true })
      this.state.comparePlastics.filter((plastic) =>
        plastic.id === 1 ? this.setState({ mapPlastic: plastic }) : null,
      )
    }

    if (
      e.target.className === 'plastic cups afterCup' ||
      e.target.className === 'img b-cup afterCupImg'
    ) {
      this.setState({ modalIsOpen: true })
      this.state.comparePlastics.filter((plastic) =>
        plastic.id === 4 ? this.setState({ mapPlastic: plastic }) : null,
      )
    }

    if (
      e.target.className === 'plastic can afterCan' ||
      e.target.className === 'img b-can afterCanImg'
    ) {
      this.setState({ modalIsOpen: true })
      this.state.comparePlastics.filter((plastic) =>
        plastic.id === 5 ? this.setState({ mapPlastic: plastic }) : null,
      )
    }
  }
  ...
  render() {
    ...
    return (
      <>
        <div className="desktop-view-msg-overlay">
          <div className="desktop-view-msg">
            <h1>Hi {this.props.name.length? this.props.name : 'there'} !</h1>
            <h2>
 
              Experince Jacks journey and learn about how plastic pollution is
              effecting Jack and his sea turtle friends!
            </h2>
            <h3>
              Get the best experience on desktop screen minimum width: 1440px and
              height: 764px or above 😊
            </h3>
            <Turtle name={this.props.name} class={'mobile'}/>
          </div>
        </div>
        <Modals {...this.state} handleClose={this.handleClose} />
        <Collected {...this.state} />
        <Turtle name={this.props.name} class={'not-mobile'}/>
        <div className="outer-wrapper" onScroll={this.listenScrollEvent}>
          <div className="wrapper">
            ...
            >
              <img className="img b-bag " src={bag} alt="" />
            </div>
            <br />
            <div
            ...
            >
              {' '}
              <img className="img b-bottle " src={bottle} alt="" />
            </div>
            <br />
            <div
            ...
            >
              {' '}
              <img className="img b-straw" src={straw} alt="" />
            </div>
            <br />
            <div
              className="plastic cups"
              onMouseDown={this.handleMousedown}
              onClick={this.handleClick}
            >
              {' '}
              <img className="img b-cup" src={cup} alt="" />
            </div>
            <br />
            <div
              className="plastic can"
              onMouseDown={this.handleMousedown}
              onClick={this.handleClick}
            >
              {' '}
              <img className="img b-can" src={can} alt="" />
            </div>
            ...
            <div className="slide two">
              <p className="collectfirstplastic">collect me!→</p>{' '}
              <PlasticStats plastics={this.state.plastics} /> <Ocean />
            </div>
            <div className="slide three">
              {' '}
              <Jelly {...this.state} /> <Ocean />
            </div>
            <div className="slide four">
              <Charts {...this.state} /> <Ocean />
            </div>
            <div className="slide five">
              {' '}
              <Fishes {...this.state} /> <Ocean />
            </div>
            <img
              className="fishy twin"
              src={require('../img/greyFish.png')}
              alt=""
            />
            <div className="slide six">
              <Lastpgmessage name={this.props.name} />{' '}
            </div>
